...
$productObject = [
    ...
    [ 'label' => 'Sell Price', 'name' => 'price' ],
    [ 'label' => 'Product Category', 'name' => 'product_category' ],
    [ 'label' => 'Active/Inactive Status', 'name' => 'status' ],
];
@endphp
...
@section('content')
    <section class="place-items-center">
        <div class="xs:w-auto sm:w-[450px]">
            ...
            <form action="{{ route('products.store') }}" method="POST" class="flex flex-col gap-3">
                @csrf
                @foreach ($productObject as $product)
                    <div>
                        ...
                        @if ($product['name'] === 'product_category')
                            <div class="relative">
                                <button type="button" class="w-full outline outline-1 focus:outline-2 outline-slate-400 rounded p-2 flex flex-wrap items-center gap-1" id="dropdownButton" aria-haspopup="listbox" aria-expanded="false" aria-controls="dropdownMenu">
